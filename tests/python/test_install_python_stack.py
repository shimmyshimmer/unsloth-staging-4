...
class TestUvSafePathHardening:
    """Edge cases for the shared uv_safe_path helper + the UV_OVERRIDE channel.

    uv also truncates `--override`/`UV_OVERRIDE` at the first space, so the
    macOS-arm64 override (install_python_stack + utils.mlx_repair) must route
    through the same space-free-path helper (issue #6503).
    """

    @pytest.mark.skipif(ips.IS_WINDOWS, reason = "POSIX temp-copy fallback")
    def test_tmpdir_with_space_falls_back(self, tmp_path, monkeypatch):
        """If the temp root itself has a space, fall back to the original path
        rather than hand uv a still-broken copy under a space-containing dir."""
    ...
    def test_no_temp_dir_leak_on_copy_failure(self, tmp_path, monkeypatch):
        ...
        assert out == str(src)                      # fell back
        assert set(glob.glob(pattern)) == before    # nothing leaked
    ...
    def test_uv_override_value_is_space_safe(self, tmp_path):
        """Mirrors the macOS-arm64 UV_OVERRIDE assignment: the stored value must
        be space-free (uv truncates UV_OVERRIDE at the first space)."""
        from backend.utils import uv_path_safety as uvps
