...
def test_the_posix_installers_carry_the_same_guard(path: Path, funcs: tuple[str, ...]):
    ...
    for func in funcs:
        ...
        assert "_unsloth_conda_env_active" in body.group(0), (
            f"{func} must not prepend inside conda"
        )
...
def test_both_halves_call_the_same_environment_an_active_conda(path: Path):
    ...
    assert posix_vars == windows_vars, (
        f"{path.name} reads {sorted(posix_vars)} but install.ps1 reads "
        f"{sorted(windows_vars)}"
    )
