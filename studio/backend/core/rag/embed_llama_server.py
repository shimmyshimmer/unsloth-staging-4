...
class LlamaServerBackend:
    ...
    def __init__(self) -> None:
        ...
        # no rebuild. trust_env=False so an ambient HTTP(S)_PROXY is not applied to
        # the loopback embedder (a proxy that 503s 127.0.0.1 would break requests).
    ...
    def _wait_for_health(
        ...
        while time.monotonic() < deadline:
            if not self._process_alive():
                ...
                return False
            try:
                # trust_env=False: never route this loopback health probe through an
                # ambient HTTP(S)_PROXY (a proxy returning 503 for 127.0.0.1 would hang startup).
