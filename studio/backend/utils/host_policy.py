...
# A non-wildcard `--host` binds one interface, so a hardcoded `127.0.0.1` self-call cannot connect.
...
def self_request_host(app_state, server = None) -> str:
    """`server_request_host` is authoritative; the scope pair covers running outside run_server."""
