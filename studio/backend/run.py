...
def _install_windows_console_handler(handler) -> bool:
    ...
    Closing the window (or logging off) raises CTRL_CLOSE_EVENT, which Python
    never turns into a signal -- so no signal handler and no atexit hook runs,
    and the cooperative cleanup is skipped entirely. Windows gives the handler a
    few seconds before killing the process, which is enough to stop the sidecars
    that would otherwise be left holding the install.

    Returns True when the handler is installed. No-op off Windows.
    """
    if sys.platform != "win32":
        return False
    try:
        ...
        def _on_console_event(event: int) -> bool:
            if event in (CTRL_CLOSE_EVENT, CTRL_LOGOFF_EVENT, CTRL_SHUTDOWN_EVENT):
                try:
                    ...
                    pass
                return True
            # Ctrl+C / Ctrl+Break already arrive as Python signals; returning
            # False passes them to the next handler instead of running the
            # shutdown twice.
        ...
        if not kernel32.SetConsoleCtrlHandler(callback, True):
            ...
            return False
        # Hold a reference for the process lifetime: a collected callback would
        # leave Windows calling into freed memory.
...
def _graceful_shutdown(server = None):
    ...
    try:
        ...
        terminate_all()
        # Everything recorded is gone now, so the next startup has nothing to sweep.
        clear_breadcrumb()
    except Exception as e:
...
def run_server(
    ...
    # macOS has neither PR_SET_PDEATHSIG nor job objects, so a previous Studio
    # that crashed or was Force Quit left its llama-server (and friends) running.
    # Sweep them before spawning anything: a leftover holds VRAM, a port, and on
    # Windows the venv files an update needs to replace.
    try:
