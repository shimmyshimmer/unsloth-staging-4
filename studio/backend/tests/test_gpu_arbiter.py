...
def test_restore_owner_account_returns_residency_to_the_displaced_account(calls, monkeypatch):
    # A media register only starts the load, so a failure that leaves the previous model resident hands residency back.
    ...
    # Once restored there is nothing left to hand back.
...
def test_restore_owner_account_is_a_noop_once_someone_else_holds_residency(calls, monkeypatch):
    # A third load committed in the meantime, so the failed claimant may not rewrite its residency.
    from utils.account_context import AccountContext, run_as
