...
class SharedEngine:
    """A public resident whose generations serialize on the real backend generation slot."""
...
def test_a_queued_request_does_not_take_progress_and_cancel_from_the_active_generation(engine):
    """Alice is denoising; Bob queues behind her. Alice keeps her own progress and Stop."""
    results = {}
    ...
    try:
        with client_for(BOB) as client:
            # Bob is only waiting on the slot, so Alice's steps stay hidden from him.
