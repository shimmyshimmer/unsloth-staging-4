...
"""A managed account may only preview datasets it is allowed to see."""
...
def shared_cache(monkeypatch, tmp_path):
    ...
    # The global audio shim is irrelevant here and its install leaks across test modules.
...
def offline_hub(monkeypatch):
    """The online tiers fail, so the handler falls back to the same shared cache."""
