...
class ReservingVideoBackend:
    """Reserves the job under its own lock, then finishes resolving inputs, as the real backend does."""
    ...
    def begin_generate(self, **kwargs):
        ...
        self.reserved.set()
        # The real begin_generate keeps working after the reservation (worker start, canvas report).
...
def test_ownership_holds_from_the_reservation_not_from_the_end_of_begin_generate(backend):
    """While Bob's reservation is still inside begin_generate, the clip is already his."""
    ...
    # Alice ran a clip earlier, so the route's ownership marker names her.
    ...
    try:
        ...
        with _client(ALICE) as client:
            # Bob holds the reservation, so Alice must not be able to stop his clip.
...
def test_the_real_backend_records_the_account_inside_the_locked_reservation():
    """The reservation itself names the account, before the worker thread is started."""
    import functools
