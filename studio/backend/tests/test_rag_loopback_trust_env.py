"""Regression test for the RAG loopback trust_env fix.

The RAG embedder (embed_llama_server.py) and vision captioner (captioner.py) talk to a
LOCAL (127.0.0.1) llama-server via httpx. With httpx's default trust_env=True an ambient
HTTP(S)_PROXY that 503s loopback breaks embedder startup / captioning. Every httpx client
or call in the RAG package targets that loopback server, so all of them must pin
trust_env=False. This AST test locks that in (same approach as
test_inference_dispatcher_resilience.py for the route-level clients).
"""
import ast
import os
