...
def test_a_private_base_repo_of_a_shared_resident_is_reauthorized(monkeypatch):
    """Bob's public pick assembled with his private base is not Alice's to generate on."""
    used = []
    status = {
        "loaded": True,
    ...
    assert used == []
    # The account the base repo was downloaded by still generates on its own resident.
...
def test_baked_private_adapters_of_a_shared_resident_are_reauthorized(monkeypatch):
    ...
    assert used == []
    # A resident that is no longer the recorded one keeps the stale components out of the way.
...
def test_a_public_replacement_is_regenerated_after_reauthorization(monkeypatch):
    """The retry is one reauthorized attempt, not a refusal, when the new resident is allowed."""
    used = []
    resident = {
        "loaded": True,
        "repo_id": "org/public-model",
...
def test_the_gallery_persist_window_still_belongs_to_the_generating_account(monkeypatch):
    """Ownership has to outlast generate(): the records are written after it returns."""
    status = {
        "loaded": True,
        "repo_id": "org/public-model",
        "family": "z-image",
