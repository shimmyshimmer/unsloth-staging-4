...
unsafe fn job_process_ids(job: &OwnedHandle) -> std::io::Result<Vec<u32>> {
    // The list is variable length, so query with a generous fixed-size buffer and
    // grow once if the job holds more entries than it can carry.
    ...
    {
        // ERROR_MORE_DATA still fills the buffer with as many ids as fit, which is
        // all this drain needs: whatever is left over gets another pass below.
...
/// Terminate every other process in the app job.
///
/// Called immediately before `suspend_for_update_installer`: once kill-on-close
/// is cleared, nothing reaps the job, so anything the cooperative stop path
/// missed (a sandbox grandchild, a sidecar that ignored its stop request) would
/// survive the app and hold the venv open against the installer. Returns the
/// number of processes it had to terminate.
#[cfg(windows)]
pub fn drain_job_children() -> std::io::Result<usize> {
    ...
    for id in ids {
        unsafe {
            ...
            if handle.is_null() {
                // Already gone, or not ours to kill; either way it is not a leak
                // this pass can do anything about.
                continue;
            }
...
/// immediately before it launches the installer. Drains the job first: clearing
/// kill-on-close removes the last backstop, and a child that outlives the app
/// keeps `%STUDIO_HOME%\unsloth_studio` open, which is exactly what makes the
/// next `unsloth studio update` refuse to run.
#[cfg(windows)]
...
/// Re-arm the crash-cleanup guarantee after a suspension that did not lead to
/// an exit (the installer failed to launch, or the update was abandoned).
///
/// Without this the app keeps running with no reaper for the rest of its life,
/// and every child it spawns from then on leaks on a crash or an End Task.
#[cfg(windows)]
