...
async def multi_turn_chat(
    ...
    idx = 1
    await sp.screenshot(r.shot(sp, idx, "chat_open"))
    idx += 1
    await pick_model(sp, model)
    await sp.screenshot(r.shot(sp, idx, "model_picked"))
    idx += 1
    for i, prompt in enumerate(prompts, start = 1):
        await send_prompt(sp, prompt)
        await sp.screenshot(r.shot(sp, idx, f"turn_{i:02d}_sent"))
        idx += 1
        await wait_for_stream(sp, timeout_ms = settle_timeout_ms)
        await sp.screenshot(r.shot(sp, idx, f"turn_{i:02d}_done"))
        idx += 1
...
async def vision_upload(
    sp: StudioPage, model: str, image_path: Path, prompt: str, out_dir: Path
) -> FlowResult:
