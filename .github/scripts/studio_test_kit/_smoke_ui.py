...
async def main() -> int:
    ...
    try:
        auth = StudioAuth(
            access_token = "A.B.C", refresh_token = "R.E.F", base_url = f"http://127.0.0.1:{port}"
        )
        ...
        assert sp.video_mp4 is not None and sp.video_mp4.exists(), f"mp4 missing: {sp.video_mp4}"
        assert sp.video_mp4.stat().st_size > 0
        print(
            f"OK -- init script primed token; screenshot {shot.stat().st_size}B; "
