...
async def open_chat(
    ...
    try:
        async with async_playwright() as p:
            ...
            # Studio serves `connect-src 'self'`, so an init script cannot
            # reach a helper server on another port; the fetch is blocked and
            # silently delivers nothing. Only for scenes that inject one.
            if bypass_csp:
                kw["bypass_csp"] = True
