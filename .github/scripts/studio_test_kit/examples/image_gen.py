...
async def main() -> None:
    ...
    providers = [gemini_provider(api_key = os.environ["GEMINI_API_KEY"], models = [args.model])]
    ...
    r.attach_video(sp)
    print(f"Image saved to {r.artefacts['image_path']} " f"({r.artefacts['image_bytes']} bytes)")
    if r.video_webm:
