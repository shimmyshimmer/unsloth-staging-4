...
async def drive(
    port: int, password: str, model: str, out_dir: Path, video_dir: Path, video_name: str
) -> tuple[list[Path], Path]:
    base = f"http://127.0.0.1:{port}"
    auth = await login(base, "unsloth", password)
    providers = [gemini_provider(api_key = os.environ["GEMINI_API_KEY"], models = [model])]
...
async def main() -> None:
    ...
    ap.add_argument("--studio-home-root", type = Path, default = Path("temp/side_by_side_studios"))
    args = ap.parse_args()
    ...
    post_shots, post_video = await drive(
        args.post_port,
        post.bootstrap_password or "",
        args.model,
        args.root / "post",
        args.root / "post" / "video",
        "post",
    )
    ...
    for i, (left, right) in enumerate(zip(pre_shots, post_shots), start = 1):
        hstack_images(
            left,
            right,
            combined / f"sxs_{i:02d}.png",
            label_left = args.pre_branch,
            label_right = args.post_branch,
        )
