"""DPO (or ORPO) preference sample job.

    python jobs/dpo.py --max-steps 3                 # unsloth/Qwen3-0.6B, PatchDPOTrainer + DPOTrainer
    python jobs/dpo.py --orpo --max-steps 3          # ORPOTrainer
    python jobs/dpo.py --tiny                        # tiny model, CPU / CI smoke

Zephyr_(7B)-DPO / Llama3_(8B)-ORPO notebook settings (beta 0.1, ref_model=None so the frozen
base is the reference), on an in-code preference fixture so base and head see identical pairs.
"""
...
with c.JobRecorder(a) as rec:
    ...
    ds = Dataset.from_list(preference_fixture(a.rows))  # DPO/ORPO tokenize_row appends EOS itself
    ...
    if not a.orpo:
        # DPO starts at log(2) with a frozen reference equal to the policy; margins must be finite.
