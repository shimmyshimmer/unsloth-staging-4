"""GRPO sample job.
    ...
    python jobs/grpo.py --max-steps 3 --fast-inference       # notebook preset, vLLM (Linux CUDA)
    python jobs/grpo.py --preset smoke --fast-inference      # tests/fast_inference/test_fast_inference.py shape
    python jobs/grpo.py --tiny                               # tiny model, smoke preset, CPU / CI

notebook: Qwen3_(4B)-GRPO recipe (custom chat template, step-capped SFT format priming on
unsloth/OpenMathReasoning-mini, open-r1/DAPO-Math-17k-Processed, the 4 notebook reward functions).
smoke: fixed prompts + a length reward with a tie-breaker so every group has reward_std > 0; a
random tiny model never earns the math rewards, so it is the only preset --tiny can use.
"""
...
p.add_argument("--preset", choices=("notebook", "smoke"), default=None, help="default: smoke with --tiny, else notebook")
p.add_argument("--fast-inference", action="store_true", help="vLLM generation (Unsloth fast_inference=True)")
...
p.add_argument("--prime-steps", type=int, default=5, help="notebook SFT format-priming steps, 0 skips")
p.add_argument("--rows", type=int, default=64)
p.set_defaults(lr=5e-6, max_seq_length=2048)  # notebook value; completions still capped
a = c.resolve_args(p)
...
def smoke_reward(completions, **kw):
    # Length plus a per-position tie-breaker: rewards always differ within a group, so the
    # advantages are nonzero and every step has a real policy gradient (test_fast_inference.py).
    out = []
...
with c.JobRecorder(a) as rec:
    ...
    model, tok = k.load_text_model(a, lora_r=a.lora_r, lora_alpha=2 * a.lora_r if a.preset == "notebook" else a.lora_r, **extra)
    k.ensure_pad(tok)
    ...
    if a.preset == "notebook":
        ...
        if a.prime_steps > 0:
            ...
            # notebook priming SFTConfig: adamw_8bit, weight_decay 0.001, linear (its warmup_steps=5
            # would cover the whole capped run, so warmup stays 0)
    ...
    ga.update(per_device_train_batch_size=a.num_generations, optim="adamw_8bit" if a.preset == "notebook" and a.backend != "hf" else "adamw_torch")
    if a.preset == "notebook":  # notebook GRPOConfig schedule
        # transformers 5 dropped warmup_ratio; a float warmup_steps < 1 is the ratio there
        import inspect
    ...
    if a.preset == "smoke":
        # Same per-step asserts as tests/fast_inference/test_fast_inference.py.
    ...
    if kls:  # an unlogged kl (beta=0) is not evidence; don't record a vacuous pass
