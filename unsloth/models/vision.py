...
def _unsloth_generate_accepts_kwarg(model, key):
    # Mirror transformers `GenerationMixin._validate_model_kwargs` so we never inject a
    # generation kwarg that the top-level `generate()` validation would then reject.
    # Some fused / PEFT-wrapped models (e.g. gpt-oss) expose `logits_to_keep` only on an
    # inner submodule's forward, while transformers validates against the top-level
    # `prepare_inputs_for_generation` (plus `forward` when it takes **kwargs). Injecting the
    # kwarg there raises:
    #   ValueError: The following `model_kwargs` are not used by the model: ['logits_to_keep']
    # We therefore only inject when transformers would accept it at the top level.
    try:
...
def _install_offload_embedding_hooks(embed_tokens):
    # Run the embedding lookup on the device where the embedding WEIGHT currently lives
    # (CPU while offloaded), then move the result back to the input's device so the rest
    # of the model keeps running on GPU. The weight device is read at CALL time (not a
    # hard-coded "cpu") because a later model.to(...) can pull a non-quantized (e.g. bf16)
    # embedding back onto the GPU; a hard-coded "cpu" would then send the index to CPU
    # while the weight is on CUDA and index_select raises a device mismatch. The origin
    # device is saved on the module, not the input tensor, since the pre-hook returns a
    # NEW tensor and an attribute on the original would be lost by the time the post-hook
    # runs (why the earlier tensor-attribute approach "didn't seem to work").
            ...
            if not fast_inference:
                ...
                if offload_embedding:
                    if bool(os.environ.get("WSL_DISTRO_NAME") or os.environ.get("WSL_INTEROP")):
                        ...
                        # Device-safe embedding offload: run the lookup where the weight
                        # currently is and move the result back. See
                        # _install_offload_embedding_hooks for why this is call-time
                        # dynamic (bf16 embeddings can be pulled back onto the GPU).
